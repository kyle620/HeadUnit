...
/*
   ...
   In this example we use WiringPi to access the GPIOs of the Raspberry Pi.
*/
#include "wiringPi.h"
#include "ewrte.h"
...
/* LED has to be connected to BCM_GPIO17 and can be accessed as GPIO 0 via wiringPi */
#define LED_GPIO 0

/* Button has to be connected to BCM_GPIO22 and can be accessed as GPIO 3 via wiringPi */
...
void DeviceDriver_Initialize( void )
{
  ...
  */
  ...
  /* Configure GPIO for hardware button */
...
int DeviceDriver_ProcessData( void )
{
  ...
  */
  ...
  IsHardButtonDown = ( digitalRead( BUTTON_GPIO ) == LOW );
